...
1. 
修改，MonitoringDetailModal.tsx 的日志部分，如果日志内容中出现 【转发邮件到: [{'email': 'dlsjie@cn.ibm.com', 'name': '应收账款现金对账组'}] 】【成功获取 x 封邮件】的内容时，这部分字体加粗,高亮显示。

日志部分的修改没有生效，
我要对日志内容进行判断，当出现 【成功获取 n 封邮件】,【分类完成，共分类 n 封邮件】【邮件转发完成，共转发 n 封邮件】【转发邮件到: [{'email': 'dlsjie@cn.ibm.com', 'name': '应收账款现金对账组'}] 】【成功获取 x 封邮件】
这些内容时，要对这些内容加粗，高亮显示。
...
2. 
修改，MonitoringDetailModal.tsx 的分类统计部分，当每个分类的邮件数变化时，这个分类需要发生动画效果，能够让用户感受到这个变化，请你给我设计效果。包括但不限于，分类块（区域）闪烁，有+1的闪现效果，等等。



consumers.py 与前端通信的邮件数量，total_emails，processed_emails，forwarded_emails等，现在是从表cc_email取得，
依赖于对这个表的更新，
是否可以改成，从实际处理的代码中取得今天的取得邮件数，分类成功邮件数，转发邮件数。


websocket 后端向前端发送数据，是否有时间间隔，异步发送的控制，受什么影响，我希望后端处理完成一个邮件后，能很快的更新日志到前端显示，需要修改哪里？



2 秒的邮箱检查，刷新时间是在哪里控制的，

后端每隔2秒向前端发送日志？或者 每隔2秒刷新邮箱，是在那个代码里控制的？似乎不是 EMAIL_MONITOR_INTERVAL。，


当后端在调用邮件分类处理的代码，做邮件分类的时候，为啥知道处理完成，前端才能收到日志，并且如果分类的邮件过多的时候，前端就不能收到日志了。

我要修改成，分类处理过程中，也向前端发送日志。帮我检查现在的代码实现，设计一个新的实现方式。
...
检查代码，告诉我，
1. 邮件接收，读取，分类，转发，这一系列过程的，各个文件，方法的调用关系。
2. 我要改成接收邮件后，分类处理做成多线程并行的形式，该怎么修改，代价最小。并且日志不能混乱。


检查代码，总结，邮件接收，读取，分类，转发，这一系列过程的，各个文件，方法的调用关系。

把邮件分类处理，改成多线程并行调用的形式。

要求，
不要影响现在的既存的日志内容，通过websocket发送给前端的日志不要改动，
如有需要，可以添加新的日志内容。
每个线程的日志不要混乱，要可以区分。
要保持现在的代码结构，接口方式，不要大改。
如有必要，转发部分可以在分类后一起执行。
确保 WebSocket 连接需要能够处理并发的日志消息。


还是有问题，读取邮件的时候，也没有判断，相同的邮件被反复读取。应该读取一次，分类，转发，对应的状态保存到ccemail表。
是否可以改成，统一读取邮件，没有被处理的邮件，被读入。然后，并发多线程对邮件做分类转发，保存db。
这一次处理循环结束后，间隔指定时间，执行下一个处理循环。